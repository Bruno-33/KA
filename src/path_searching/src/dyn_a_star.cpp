...
// bool AStar::check_kinodynamic(Vector3i& a, Vector3i& b, Vector3i& c)
bool AStar::check_kinodynamic(Vector3i a, Vector3i b, Vector3i c)
{
    Vector3i delta_ba = a - b,delta_bc = c - b; 
    return delta_ba.dot(delta_bc)<0;
}
        ...
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
                for (int dz = -1; dz <= 1; dz++)
                {
                    ...
                    if(current->cameFrom != NULL && check_kinodynamic(current->cameFrom->index,current->index,neighborIdx) == false) continue;
