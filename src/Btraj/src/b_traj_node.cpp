...
{   
    ...
    ***/
    ...
    if (a_star_->AstarSearch(/*(in-out).norm()/10+0.05*/ 0.1, _start_pt, _end_pt))
    {
...
void timeAllocation(vector<Cube> & corridor,vector<Vector3d>& grid_path)
{   
    double my_base_time = _resolution/_MAX_Vel/0.5;
    int in=-1,out=-1;
...
int main(int argc, char** argv)
{
    ...
    //     for(int j=-1;j<2;++j){
    //         for(int k=-1;k<2;++k){
    //             if(a_star_->check_kinadynamic(Vector3i(0,0,0),Vector3i(0,1,1),Vector3i(0+i,1+j,1+k))){
    //                 cout<<i<<" "<<j<<" "<<k<<" "<<endl;
    //             }
    //         }
    //     }
